...
public class CalendarUtil {
     ...
     */
    public static String solarToLunar(String solarStrDate) {
        String[] dataStrArr = solarStrDate.split("-");
        int[] dataArr = solarToLunar(Integer.parseInt(dataStrArr[0]), Integer.parseInt(dataStrArr[1]),
                Integer.parseInt(dataStrArr[2]));
        String month = String.valueOf(dataArr[1]);
        if (month.length() != 2) {
            month = "0" + month;
        }

        String day = String.valueOf(dataArr[2]);
        if (day.length() != 2) {
            day = "0" + day;
        }

        return dataArr[0] + "-" + month + "-" + day;
    }
     ...
     * @param lunarStrDate
     *            阴历日期 (yyyy-MM-dd)
     * @return 阳历日期 (yyyy-MM-dd)
     */
    public static String lunarToSolar(String lunarStrDate) {
        String[] dataStrArr = lunarStrDate.split("-");
        // 如果遇到润月去第一个月
        int[] dataArr = lunarToSolar(Integer.parseInt(dataStrArr[0]), Integer.parseInt(dataStrArr[1]),
                Integer.parseInt(dataStrArr[2]),
                false);
        String month = String.valueOf(dataArr[1]);
        if (month.length() != 2) {
    ...
    public static void main(String[] args) {
        System.out.println(solarToLunar("1992-06-30"));
        System.out.println(lunarToSolar("1992-06-01"));
        System.out.println(lunarToSolar("2017-06-01"));
    }
